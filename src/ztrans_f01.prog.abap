...
*&---------------------------------------------------------------------*
FORM lxe_t002_check_language .
  SELECT SINGLE laiso INTO @DATA(lv_laiso) FROM t002 WHERE spras = @pm_slang.
  CHECK sy-subrc = 0.
  gv_sname = |TXT_{ lv_laiso }|.

  SELECT SINGLE laiso INTO @lv_laiso FROM t002 WHERE spras = @pm_tlang.
  CHECK sy-subrc = 0.
  gv_tname = |TXT_{ lv_laiso }|.
  "ISO 언어 코드로 변환
...
* 확인
  ...
  LOOP AT gt_110 WHERE mark = 'X'.
    ...
    DELETE FROM zttrans WHERE erdat = gt_110-erdat
                            AND erzet = gt_110-erzet
                            AND seqno = gt_110-seqno.
    IF sy-subrc <> 0.
      ROLLBACK WORK.
      MESSAGE e000 WITH 'Delete Error [ZTTRANS]'.
    ENDIF.
...
FORM alv_user_cmd_sel_all USING po_grid TYPE REF TO cl_gui_alv_grid
  ...
  LOOP AT gt_m.
    ...
    PERFORM sel_zttrans USING    gt_m-s_text
  ...
  LOOP AT pt_fcat.
    ...
    CASE pt_fcat-fieldname.
      ...
      WHEN 'S_TEXT'.
        ...
        SELECT SINGLE sptxt INTO @pt_fcat-coltext FROM t002t WHERE spras = @sy-langu AND sprsl = @pm_slang.
      WHEN 'ICO_SEL'.
        pt_fcat-col_pos    = '45'.
        pt_fcat-coltext    = '选择'.
        pt_fcat-just       = 'C'.
      ...
      WHEN 'T_TEXT'.
        pt_fcat-col_pos    = '50'.
        pt_fcat-coltext    = '目标文本'.
        pt_fcat-edit       = 'X'.
        SELECT SINGLE sptxt INTO @pt_fcat-coltext FROM t002t WHERE spras = @sy-langu AND sprsl = @pm_tlang.
      WHEN 'UNITMLT'.
        pt_fcat-col_pos    = '60'.
      WHEN 'UPPCASE'.
        pt_fcat-col_pos    = '70'.
  ...
  LOOP AT pt_fcat.
    ...
    CASE pt_fcat-fieldname.
      ...
      WHEN 'SEQNO'.
        pt_fcat-col_pos    = '90'.
      WHEN OTHERS.
        IF pt_fcat-fieldname(4) = 'TXT_'.
          SELECT SINGLE spras INTO @DATA(lv_spras) FROM t002 WHERE laiso = @pt_fcat-fieldname+4(2).
          CHECK sy-subrc = 0.
          SELECT SINGLE sptxt INTO @pt_fcat-coltext FROM t002t WHERE spras = @sy-langu AND sprsl = @lv_spras.
          pt_fcat-col_pos    = '30'.
          pt_fcat-edit       = 'X'.
          IF pm_slang NE lv_spras AND pm_tlang NE lv_spras.
            pt_fcat-no_out  = 'X'.
          ENDIF.
        ELSE.
          pt_fcat-no_out     = 'X'.
...
FORM alv_dbl_clk_1 USING p_row
  ...
  CASE p_col.
    ...
    WHEN 'ICO_SEL'.
      ASSIGN COMPONENT gv_tname OF STRUCTURE gt_110 TO FIELD-SYMBOL(<fs_text>).
      CHECK sy-subrc = 0.
      gt_m-t_text = <fs_text>.
...
FORM alv_dbl_clk_0_ico_sel USING p_row
  ...
  PERFORM sel_zttrans USING    ps_m-s_text
                        CHANGING l_t_text
                                 l_ico_sel.
...
*  REPLACE ALL OCCURRENCES OF ' ' IN L_TEXT WITH '*'.
  ...
  lv_condi = |{ gv_sname } IN SO_TEXT AND { gv_tname } NE '' |.
...
FORM at_sel_scr .
  ...
  IF so_prog[]  IS INITIAL AND
    ...
    IF so_class[] IS INITIAL.
      MESSAGE s000(zfi) WITH '请输入单个对象或包名'(m01)
                DISPLAY LIKE 'E'.
      g_error = 'X'.
      EXIT.
    ELSE.
...
FORM save_zttrans .
  ...
  DATA: lt_trans LIKE zttrans OCCURS 0 WITH HEADER LINE,
        l_seqno  TYPE zttrans-seqno.

  DATA: lv_condi TYPE string.
  DATA: lv_condi1 TYPE string.
  lv_condi = |{ gv_sname } = GT_M-S_TEXT AND { gv_tname } = GT_M-T_TEXT |.
  ...
  LOOP AT gt_m WHERE mark = 'X'
    ...
    SELECT SINGLE * FROM zttrans
     WHERE (lv_condi).

    IF sy-subrc = 0.
      IF zttrans-objname IS INITIAL.
        UPDATE zttrans SET objtype = gt_m-objtype
                             objname = gt_m-objname
                             textkey = gt_m-textkey
                       WHERE erdat   = zttrans-erdat
                         AND erzet   = zttrans-erzet
                         AND seqno   = zttrans-seqno.
      ENDIF.
      CONTINUE.
    ENDIF.

    READ TABLE lt_trans WITH KEY (gv_sname) = gt_m-s_text (gv_tname) = gt_m-t_text.
    IF sy-subrc = 0.
      CONTINUE.
    ENDIF.

    CLEAR lt_trans.
    lt_trans-erdat   = sy-datum.
    lt_trans-erzet   = sy-uzeit.
    lt_trans-seqno   = l_seqno = l_seqno + 1.
    ASSIGN COMPONENT gv_sname OF STRUCTURE lt_trans TO FIELD-SYMBOL(<fs_s_text>).
    IF sy-subrc = 0.
      <fs_s_text> = gt_m-s_text.
    ENDIF.
    ASSIGN COMPONENT gv_tname OF STRUCTURE lt_trans TO FIELD-SYMBOL(<fs_t_text>).
    IF sy-subrc = 0.
      <fs_t_text> = gt_m-t_text.
    ENDIF.
    lt_trans-objtype = gt_m-objtype.
    lt_trans-objname = gt_m-objname.
    lt_trans-textkey = gt_m-textkey.
    APPEND lt_trans.

  ENDLOOP.

  IF lt_trans[] IS INITIAL. EXIT. ENDIF.

  MODIFY zttrans FROM TABLE lt_trans.

  IF sy-subrc <> 0.
    ROLLBACK WORK.
    MESSAGE e000 WITH 'Save Error [ZTTRANS]'.
  ENDIF.

  COMMIT WORK.
  MESSAGE s000 WITH 'Success'.

ENDFORM. " SAVE_ZTTRANS
...
*&      Form  SEL_ZTTRANS
*&---------------------------------------------------------------------*
FORM sel_zttrans USING p_s_text
                   CHANGING p_t_text
                            e_icon.
  ...
  lv_condi = |{ gv_sname } = l_s_txt AND { gv_tname } NE ''|.

  SELECT *
  INTO CORRESPONDING FIELDS OF TABLE lt_trans
  FROM zttrans
  WHERE (lv_condi).
  ...
  IF lines( lt_trans ) = 1.
    ...
    ASSIGN COMPONENT gv_tname OF STRUCTURE lt_trans TO FIELD-SYMBOL(<fs_trans>).
    CHECK sy-subrc = 0.
    p_t_text = <fs_trans>.
    EXIT.
  ENDIF.
...
* 수정된 건
  ...
  LOOP AT gt_110 WHERE edit_gb = 'X'.
    CLEAR lt_trans.
    MOVE-CORRESPONDING gt_110 TO lt_trans.
    APPEND lt_trans.
  ENDLOOP.

  IF lt_trans[] IS INITIAL.
    MESSAGE s000 WITH '没有修改项'.
    EXIT.
  ENDIF.

* 확인
  DATA: l_answer.
  PERFORM popup_conf USING    '是否保存修改？'
                     CHANGING l_answer.
  IF l_answer <> '1'. LEAVE SCREEN. ENDIF.
